...
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, string prevPaths) const
{
	// Base case
	if (subRoot == NULL)
		return;
	string currPaths = prevPaths + " " + to_string(subRoot->elem);
	if (subRoot->left == NULL && subRoot->right == NULL) 
	{
		cout << "Path: "<< currPaths<<endl;
		return;
	}
	
	printPaths(subRoot->left, currPaths);
	printPaths(subRoot->right, currPaths);
...
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot,int countPaths) const
{
	// Base case
	if (subRoot == NULL)
		return 0;
	return countPaths + sumDistances(subRoot->left, countPaths + 1) + sumDistances(subRoot->right, countPaths + 1);
